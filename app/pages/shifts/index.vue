    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-slate-500 mt-1">
          Seguimiento de sesiones de turno
        </p>
      </div>
      <AppButton
        variant="primary"
        @click="router.push('/shifts/create')"
      >
        <svg
          class="h-4 w-4"
          fill="none"
    ...
    <AppCard padding="none">
      ...
      <div class="p-4">
        <AppTable
          ...
          :rows="shifts"
          :loading="loading"
          empty-message="No se encontraron turnos."
        >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-2">
              <AppButton
                size="sm"
                variant="ghost"
                @click="router.push(`/shifts/${row.id}`)"
              >
                Ver
              </AppButton>
              <AppButton
                size="sm"
                variant="outline"
                @click="router.push(`/shifts/${row.id}/edit`)"
              >
                Editar
              </AppButton>
              <AppButton
